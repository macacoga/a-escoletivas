...
class ExtractiveSummarizer(LoggerMixin):
    ...
    def __init__(self):
        ...
        self.legal_keywords = {
            ...
            "worker_rights": [
                ...
                "gratificação",
                "diárias",
                "custos de deslocamento",  # Adicionados termos mais específicos
            ],
            "process_actors_and_parties": [  # Renomeado e expandido para maior clareza
                "autor",
                "réu",
                "reclamante",
                "reclamado",
            ...
            "judicial_entities": [  # Nova categoria para tribunais, varas, etc.
                "Tribunal",
                "Vara do Trabalho",
                "TRT",
                "TST",
            ...
            "time_references": [  # Nova categoria para termos de tempo importantes
                "prazo",
                "período",
                "data",
                "termo inicial",
        ...
        self.important_sentence_patterns = [
            # Palavras-chave de decisão e seus resultados (verbos de julgamento e desfechos)
            r"(?:julgo|decido|determino|condeno|absolvo|defiro|indefiro|acolho|rejeito|homologo|concedo|reconheço|confirmo|mantenho|reformo|extingo|declaro|arbitro|anulo)",
            # Resultados da decisão (procedência, improcedência, parcialidade)
            r"(?:procedente|improcedente|parcialmente\s+procedente|procedência|improcedência)",
            # Expressões de direito e obrigatoriedade
            r"(?:faz\s+jus|tem\s+direito|é\s+devido|deverá\s+pagar|fica\s+obrigado|cabe\s+ao\s+\w+)",  # Expandido 'cabe ao...'
            # Referências a valores monetários
            r"(?:valor\s+de|quantia\s+de|importância\s+de|montante\s+de|importe\s+de|R\$\s*\d)",  # Inclui 'R$ ' seguido de dígito para pegar a frase com o valor
            # Referências a normas jurídicas e doutrina
            r"(?:nos\s+termos\s+do|conforme\s+o|em\s+observância\s+ao)?\s*(?:Art\.|Artigo|Lei|Súmula|OJ|Decreto|CLT|CF|CC|CPC|Doutrina|Jurisprudência)",  # Mais abrangente para base legal
            # Referências a fases e documentos processuais
            r"(?:sentença|acórdão|decisão|recurso|processo|autos|manifestação|contestação)",
            # Seções importantes do documento (cabeçalhos)
            r"(?:fundamentação|ementa|dispositivo|relatório|voto|mérito|preâmbulo|conclusão)",
            # Termos indicativos de prova e fato
            r"(?:prova|laudo|perícia|testemunha|documento|ônus\s+da\s+prova|restou\s+comprovado|não\s+comprovou)",
            # Termos relacionados à rescisão ou vínculo
            r"(?:rescisão\s+indireta|justa\s+causa|sem\s+justa\s+causa|vínculo\s+empregatício|relação\s+de\s+emprego)",  # Novo padrão
        ]
    ...
    def create_summary(
        ...
        try:
            ...
            if method == "hybrid":
                summary_result = self._create_hybrid_summary(
                    processed_text, max_sentences
                )
            else:
    ...
    def _create_single_method_summary(
        self, text: str, max_sentences: int, method: str
    ) -> Dict[str, Any]:
    ...
    def _create_hybrid_summary(self, text: str, max_sentences: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for sentence_data in all_sentences:
                sentence_data["score"] += self._calculate_legal_importance(
                    sentence_data["text"]
                )
            ...
            selected_sentences = self._reorder_by_original_position(
    ...
    def _reorder_by_original_position(
        self, original_text: str, sentences: List[Dict]
    ) -> List[Dict]:
        ...
        try:
            for sentence_data in sentences:
                ...
                sentence_data["original_position"] = (
                    position if position != -1 else float("inf")
                )
    ...
    def _calculate_summary_metrics(
        ...
        try:
            ...
            compression_ratio = (
                summary_words / original_words if original_words > 0 else 0
            )
            sentence_ratio = (
                summary_sentences / original_sentences if original_sentences > 0 else 0
            )
    ...
    def create_structured_summary(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for section_name, section_text in sections.items():
                if (
                    len(section_text) > 100
                ):  # Só sumarizar seções com conteúdo suficiente
    ...
    def _identify_sections(self, text: str) -> Dict[str, str]:
        ...
        try:
            ...
            section_patterns = {
                "ementa": r"(EMENTA|E M E N T A)(.*?)(?=RELATÓRIO|RELATORIO|VOTO|ACÓRDÃO|ACORDAO|PREÂMBULO|PREAMBULO|PODER JUDICIÁRIO|$)",
                "relatorio": r"(RELATÓRIO|RELATORIO|I\s*-\s*RELATÓRIO|I\s*-\s*RELATORIO)(.*?)(?=FUNDAMENTAÇÃO|FUNDAMENTACAO|VOTO|MÉRITO|MERITO|II\s*-\s*FUNDAMENTAÇÃO|II\s*-\s*FUNDAMENTACAO|DISPOSITIVO|DISPOSIÇÃO|DISPOSICAO|III\s*-\s*DISPOSITIVO|$)",
                "fundamentacao": r"(FUNDAMENTAÇÃO|FUNDAMENTACAO|VOTO|MÉRITO|MERITO|II\s*-\s*FUNDAMENTAÇÃO|II\s*-\s*FUNDAMENTACAO)(.*?)(?=DISPOSITIVO|DISPOSIÇÃO|DISPOSICAO|III\s*-\s*DISPOSITIVO|CONCLUSÃO|CONCLUSAO|ANTE\s+O\s+EXPOSTO|DECISÃO|DECISAO|$)",
                "dispositivo": r"(DISPOSITIVO|DISPOSIÇÃO|DISPOSICAO|III\s*-\s*DISPOSITIVO|DECISÃO|DECISAO|CONCLUSÃO|CONCLUSAO|ANTE\s+O\s+EXPOSTO|PELO\s+EXPOSTO|ISTO\s+POSTO)(.*?)(?=$)",
                "preambulo": r"(ACÓRDÃO|ACORDAO|Vistos|Trata-se de|PODER\s+JUDICIÁRIO)(.*?)(?=RELATÓRIO|RELATORIO|EMENTA|E\s*M\s*E\s*N\s*T\s*A|$)",  # Para pegar o início antes do relatório/ementa
            }
